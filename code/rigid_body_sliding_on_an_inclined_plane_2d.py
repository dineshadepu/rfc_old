...
class RigidFluidCoupling(Application):
    ...
    def create_particles(self):
        ...
        xw, yw = get_2d_block(self.fluid_spacing, 20., self.tank_layers*self.tank_spacing)
        zw = np.zeros_like(xw)
        ...
        xb, yb, zb = rotate(xb, yb, zb, angle=-45.)
        ...
        body = get_particle_array(name='body',
                                  ...
                                  m_fluid=1.,
                                  rad_s=rad_s)
        # body.x[:] += np.max(body.x) - np.min(xb)
        body.y[:] += self.body_spacing * 15.
