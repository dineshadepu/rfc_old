...
# ========================================================
# Modified Mohseni equation
# ========================================================
class ModifiedMohseniCopySourceDistance(Equation):
    def initialize(self, d_idx,
                   d_m,
                   d_contact_force_dist,
                   d_contact_force_dist_tmp,
        ...
        for i in range(d_total_no_bodies[0]):
            t2 = t1 + i
            d_contact_force_dist_source[t2] = 0.
    ...
    def loop(self, d_idx,
             ...
             s_dem_id,
             d_total_no_bodies,
             d_contact_force_normal_x_source,
             d_contact_force_normal_y_source,
             d_contact_force_dist_source,
             s_contact_force_dist,
             s_contact_force_normal_x,
             s_contact_force_normal_y,
             d_normal, dt, t, WIJ, RIJ, XIJ):
        i, t1, t2, t3, t4, t5 = declare('int', 6)

        if s_contact_force_is_boundary[s_idx] == 1.:
            if d_dem_id[d_idx] != s_dem_id[s_idx]:
                t1 = d_total_no_bodies[0] * d_idx
                t2 = t1 + s_dem_id[s_idx]
                t4 = d_total_no_bodies[0] * s_idx
                t5 = t4 + d_dem_id[d_idx]

                if RIJ == d_closest_point_dist_to_source[t2]:
                    d_contact_force_dist_source[t2] = s_contact_force_dist[t5]
                    d_contact_force_normal_x_source[t2] = s_contact_force_normal_x[t5]
                    d_contact_force_normal_y_source[t2] = s_contact_force_normal_y[t5]


class ModifiedMohiseniComputeContactForce(Equation):
    """Shoya Mohseni Mofidi, Particle Based Numerical Simulation Study of Solid
    Particle Erosion of Ductile Materials Leading to an Erosion Model, Including
    the Particle Shape Effect
    ...
    def __init__(self, dest, sources, fric_coeff=0.5, kr=1e5, kf=1e3):
        ...
        self.kf = kf
        self.fric_coeff = fric_coeff
        super(ModifiedMohiseniComputeContactForce, self).__init__(dest, sources)
    ...
    def post_loop(self,
                  ...
                  d_contact_force_normal_x_source,
                  d_contact_force_normal_y_source,
                  d_contact_force_normal_z_source,
                  dt, t):
        i, t1, t2 = declare('int', 3)
        t1 = d_total_no_bodies[0] * d_idx
        ...
        for i in range(d_total_no_bodies[0]):
            t2 = t1 + i
            tmp = (d_contact_force_dist[t2] + d_contact_force_dist_source[t2]) / 2.
            if tmp < d_spacing0[0]:
                overlap = d_spacing0[0] - tmp
            else:
                overlap = d_spacing0[0]
            ...
            if overlap > 0. and overlap != d_spacing0[0]:
                ...
                tmp_ni_x = d_contact_force_normal_x[t2] - d_contact_force_normal_x_source[t2]
                tmp_ni_y = d_contact_force_normal_y[t2] - d_contact_force_normal_y_source[t2]
                tmp_ni_z = d_contact_force_normal_z[t2] - d_contact_force_normal_z_source[t2]
                tmp_magn = (tmp_ni_x**2. + tmp_ni_y**2. + tmp_ni_z**2.)**0.5

                ni_x = tmp_ni_x / tmp_magn
                ni_y = tmp_ni_y / tmp_magn
                ni_z = tmp_ni_z / tmp_magn
